...
import { Mic, MicOff, Square, RotateCcw, ArrowRight, Check, Award, TrendingUp, Volume2, Clock, Sparkles, AlertCircle, User, Briefcase, DollarSign, Heart, Target } from 'lucide-react';
...
const STOP_WORDS = new Set([
    ...
    'only', 'own', 'same', 'so', 'than', 'too', 'very', 'just', 'now', 'from',
]);
...
export default function HRInterviewPage() {
    ...
    const finishInterview = useCallback(async (allAnswers) => {
        ...
        if (user) {
            const { error } = await supabase.from('interview_sessions').insert({
                ...
                answers: allAnswers,
                feedback: { summary: 'HR interview feedback', avg_score: avgScore },
                status: 'completed',
            });
            if (!error) {
    ...
    if (!isSupported) {
        return (_jsxs("div", { className: "max-w-3xl mx-auto animate-fade-in", children: [_jsxs("div", { className: "mb-6", children: [_jsx("h1", { className: "font-display font-bold text-2xl sm:text-3xl text-ink-950", children: "HR Interview" }), _jsx("p", { className: "text-ink-500 mt-1", children: "Practice behavioral and HR questions by speaking your answers." })] }), _jsxs(Card, { className: "text-center py-12", children: [_jsx("div", { className: "w-16 h-16 rounded-2xl bg-sky-100 flex items-center justify-center mx-auto mb-4", children: _jsx(MicOff, { className: "w-8 h-8 text-sky-600" }) }), _jsx("h2", { className: "font-display font-semibold text-lg text-ink-900", children: "Voice Recognition Unavailable" }), _jsx("p", { className: "text-ink-500 text-sm mt-2 max-w-md mx-auto", children: "Your browser doesn't support speech recognition. Please use Chrome or Edge, or try the text-based AI Mock Interview." }), _jsx(Button, { className: "mt-6", onClick: () => navigate('/app/mock-interview'), children: "Go to Mock Interview" })] })] }));
    }
    if (stage === 'setup') {
        return (_jsxs("div", { className: "max-w-4xl mx-auto animate-fade-in", children: [_jsxs("div", { className: "mb-6", children: [_jsx("h1", { className: "font-display font-bold text-2xl sm:text-3xl text-ink-950", children: "HR Interview Practice" }), _jsx("p", { className: "text-ink-500 mt-1", children: "Master behavioral and HR questions by speaking your answers aloud." })] }), _jsxs(Card, { className: "mb-6", children: [_jsx("h2", { className: "font-display font-semibold text-lg text-ink-900 mb-4", children: "Choose an HR Topic" }), _jsx("div", { className: "grid sm:grid-cols-2 lg:grid-cols-3 gap-3", children: hrTopics.map((t) => {
                                const Icon = iconMap[t.icon] || User;
                                return (_jsxs("button", { onClick: () => setTopic(t.id), className: cn('p-4 rounded-xl border-2 text-left transition-all', topic === t.id ? 'border-sky-400 bg-sky-50' : 'border-ink-200 hover:border-ink-300'), children: [_jsx("div", { className: cn('w-10 h-10 rounded-lg flex items-center justify-center mb-3', topic === t.id ? 'bg-sky-500' : 'bg-ink-100'), children: _jsx(Icon, { className: cn('w-5 h-5', topic === t.id ? 'text-white' : 'text-ink-600') }) }), _jsx("p", { className: "font-medium text-sm text-ink-900", children: t.name }), _jsxs("p", { className: "text-xs text-ink-500 mt-0.5", children: [(topicQuestions[t.id] || []).length, " questions"] })] }, t.id));
                            }) })] }), _jsx(Card, { className: "mb-6 bg-sky-50 border-sky-200", children: _jsxs("div", { className: "flex items-start gap-3", children: [_jsx(AlertCircle, { className: "w-5 h-5 text-sky-600 flex-shrink-0 mt-0.5" }), _jsxs("div", { children: [_jsx("p", { className: "text-sm font-medium text-sky-800", children: "Voice-Only Mode & Microphone Permission" }), _jsx("p", { className: "text-xs text-sky-700 mt-1", children: "This is a voice-only interview. When you click the mic, your browser will ask for microphone access \u2014 click \"Allow\". Use Chrome or Edge for best results." })] })] }) }), _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-2 text-sm text-ink-500", children: [_jsx(Clock, { className: "w-4 h-4" }), "3 questions \u2022 5-10 minutes"] }), _jsxs(Button, { size: "lg", onClick: startInterview, children: [_jsx(Sparkles, { className: "w-4 h-4" }), "Start HR Interview"] })] })] }));
    }
    if (stage === 'interview') {
        const progress = ((currentIdx + 1) / questions.length) * 100;
        const isListening = micState === 'listening';
        return (_jsxs("div", { className: "max-w-3xl mx-auto animate-fade-in", children: [_jsxs("div", { className: "mb-6", children: [_jsxs("div", { className: "flex items-center justify-between mb-3", children: [_jsxs("div", { children: [_jsxs("h1", { className: "font-display font-bold text-xl text-ink-950", children: ["Question ", currentIdx + 1, " of ", questions.length] }), _jsx("p", { className: "text-sm text-ink-500", children: hrTopics.find((t) => t.id === topic)?.name })] }), _jsx(Badge, { color: "sky", children: "HR Round" })] }), _jsx(ProgressBar, { value: progress, color: "sky" })] }), _jsx(Card, { className: "mb-4", children: _jsxs("div", { className: "flex items-start gap-3", children: [_jsx("div", { className: "w-10 h-10 rounded-xl bg-gradient-to-br from-sky-500 to-brand-400 flex items-center justify-center flex-shrink-0", children: _jsx(Mic, { className: "w-5 h-5 text-white" }) }), _jsxs("div", { children: [_jsx("p", { className: "text-xs text-sky-600 font-medium mb-1", children: "HR Interviewer" }), _jsx("p", { className: "text-ink-900 font-medium leading-relaxed", children: questions[currentIdx].q })] })] }) }), _jsxs(Card, { children: [_jsxs("div", { className: "flex flex-col items-center py-8", children: [_jsx("button", { onClick: isListening ? stop : start, className: cn('w-20 h-20 rounded-full flex items-center justify-center transition-all duration-300', isListening
                                        ? 'bg-red-500 animate-pulse-glow scale-110'
                                        : 'bg-sky-500 hover:bg-sky-600 shadow-glow-sky hover:scale-105'), children: isListening ? _jsx(Square, { className: "w-8 h-8 text-white" }) : _jsx(Mic, { className: "w-8 h-8 text-white" }) }), _jsx("p", { className: "mt-4 text-sm font-medium text-ink-700", children: isListening ? `Recording… ${fmtTime(recordingTime)}` : 'Click to start recording' }), micError && (_jsxs("p", { className: "mt-2 text-xs text-red-600 flex items-center gap-1", children: [_jsx(AlertCircle, { className: "w-3.5 h-3.5" }), " ", micError] }))] }), transcript && (_jsxs("div", { className: "mt-4 bg-ink-50 rounded-xl p-4", children: [_jsx("p", { className: "text-xs text-ink-500 mb-2", children: "Live Transcript" }), _jsx("p", { className: "text-sm text-ink-700", children: transcript })] })), _jsxs("div", { className: "flex items-center justify-between mt-4", children: [_jsx(Button, { variant: "outline", onClick: () => { stop(); navigate('/app/dashboard'); }, children: "Exit" }), _jsx(Button, { onClick: submitAnswer, disabled: saving || !transcript.trim(), children: currentIdx < questions.length - 1 ? (_jsxs(_Fragment, { children: ["Next ", _jsx(ArrowRight, { className: "w-4 h-4" })] })) : (_jsxs(_Fragment, { children: ["Finish ", _jsx(Check, { className: "w-4 h-4" })] })) })] })] })] }));
    }
    return (_jsxs("div", { className: "max-w-3xl mx-auto animate-fade-in", children: [_jsxs("div", { className: "text-center mb-8", children: [_jsx("div", { className: "inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-sky-100 mb-4", children: _jsx(Award, { className: "w-8 h-8 text-sky-600" }) }), _jsx("h1", { className: "font-display font-bold text-2xl sm:text-3xl text-ink-950", children: "HR Interview Complete!" }), _jsx("p", { className: "text-ink-500 mt-1", children: "Here's how you did on your behavioral responses." })] }), _jsxs(Card, { className: "mb-6 flex flex-col items-center", children: [_jsx(ScoreRing, { score: avgScore, size: 140 }), _jsxs("div", { className: "flex gap-3 mt-6", children: [_jsxs(Button, { variant: "outline", onClick: restart, children: [_jsx(RotateCcw, { className: "w-4 h-4" }), "New Interview"] }), _jsxs(Button, { onClick: () => navigate('/app/history'), children: [_jsx(TrendingUp, { className: "w-4 h-4" }), "View History"] })] })] }), _jsxs("div", { className: "space-y-4", children: [_jsx("h2", { className: "font-display font-semibold text-lg text-ink-900", children: "Detailed Feedback" }), answers.map((a, i) => (_jsxs(Card, { children: [_jsxs("div", { className: "flex items-start justify-between gap-4 mb-3", children: [_jsxs("div", { className: "flex items-start gap-3", children: [_jsx("span", { className: "flex-shrink-0 w-7 h-7 rounded-lg bg-ink-100 flex items-center justify-center text-sm font-semibold text-ink-600", children: i + 1 }), _jsx("p", { className: "text-sm font-medium text-ink-900", children: a.question })] }), _jsx("div", { className: cn('px-2.5 py-1 rounded-lg text-sm font-semibold flex-shrink-0', a.score >= 80 ? 'bg-brand-100 text-brand-700' : a.score >= 60 ? 'bg-accent-100 text-accent-700' : 'bg-red-100 text-red-700'), children: a.score })] }), _jsxs("div", { className: "bg-ink-50 rounded-xl p-3 mb-3 flex items-start gap-2", children: [_jsx(Volume2, { className: "w-4 h-4 text-sky-500 mt-0.5 flex-shrink-0" }), _jsxs("p", { className: "text-sm text-ink-700 italic", children: ["\"", a.answer, "\""] })] }), _jsxs("div", { className: "bg-sky-50 rounded-xl p-3 mb-3 border border-sky-100", children: [_jsx("p", { className: "text-xs text-sky-600 font-medium mb-1", children: "AI Feedback" }), _jsx("p", { className: "text-sm text-ink-700", children: a.feedback })] }), _jsxs("div", { className: "bg-gradient-to-br from-brand-50 to-sky-50 rounded-xl p-3 border border-brand-100", children: [_jsx("p", { className: "text-xs text-brand-600 font-medium mb-1", children: "Model Answer" }), _jsx("p", { className: "text-sm text-ink-700 leading-relaxed", children: a.model_answer })] })] }, a.id)))] })] }));
}
